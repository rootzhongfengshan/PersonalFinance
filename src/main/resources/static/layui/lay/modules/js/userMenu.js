...
//				$wrapper.spinModal();
    ...
    function  userRole(){
        var $table4 = $('#dateTables4');
        if(_table4==null){
            _table4 = $table4.dataTable($.extend(true, {
                    "scrollY": 150,
                    "scrollX": true},
                CONSTANT.DATA_TABLES.DEFAULT_OPTION, {
                    ajax: function (data, callback, settings) {//ajax配置为function,手动调用异步查询
...
//				$wrapper.spinModal();
    ...
    function  queryRole(){
        var $table3 = $('#dateTables3');
        if(_table3==null){
            _table3 = $table3.dataTable($.extend(true, {
                    "scrollY": 190,
                    "scrollX": true},
                CONSTANT.DATA_TABLES.DEFAULT_OPTION, {
                    ajax: function (data, callback, settings) {//ajax配置为function,手动调用异步查询
...
//				$wrapper.spinModal();
    ...
    layer.queryHandle=function (obj) {
        layer.open({
            title:'<span>查看已有权限及角色</span>',
            type: 1,
            skin: 'layer-skin-news ', //加上边框
            area: ['870px', '530px'], //宽高
            content: $('#seeStaffWrap'),
            move: false,
        });
    ...
    $('#seeStaffWrap .layui-tab-ts li').click(function(){
        queryShow();
        queryRole();
    })
    $('#fpWrap .layui-tab-ts li').click(function(index){
        queryListDepart();
        userRole();
    })
//监听Tab切换，以改变地址hash值
    ...
    exports('userMenu', {}); //注意，这里是模块输出的核心，模块名必须和use时的模块名一致
});
